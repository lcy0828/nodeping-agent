...
先把 release 产物上传到静态地址，例如：

```text
https://downloads.example.com/nodeping-agent/latest.txt
https://downloads.example.com/nodeping-agent/nodeping-agent_0.1.0_checksums.txt
https://downloads.example.com/nodeping-agent/nodeping-agent_0.1.0_linux_amd64.tar.gz
```
...
默认每天 10:00 检查一次，并带 30 分钟随机延迟。更新脚本会校验 SHA256，安装新二进制，然后重启 `nodeping-agent.service`。
